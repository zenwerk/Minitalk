    ...
    case N_MESSAGE:
      ...
      if (!expression->u.message.superFlag)
      {
        code2(OP_SEND, numberArguments,
              makeLiteral(expression->u.message.selector), -numberArguments);
      }
      else
      {
...
static void codeLiteral(Node *literalNode)
{
  ...
  {
    case N_SYMBOL:
      machine.compilerLiteral = newSymbol(literalNode->u.symbol.name);
      break;
    case N_INTNUM:
      machine.compilerLiteral = newSmallInteger(literalNode->u.intnum.value);
      break;
    case N_FLONUM:
      machine.compilerLiteral = newFloat(literalNode->u.flonum.value);
      break;
    case N_STRING:
      machine.compilerLiteral = newString(literalNode->u.string.value);
      break;
    case N_CHARCON:
      machine.compilerLiteral = newCharacter(literalNode->u.charcon.value);
      break;
    case N_ARRAY:
      ...
      object = allocateObject(getPointer(machine.Array, VALUE_IN_ASSOCIATION),
                              literalNode->u.array.numberElements, true);
