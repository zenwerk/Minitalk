...
static void gl_char_cleanup() /* undo effects of gl_char_init */
{
#ifdef __unix__
#ifdef POSIX
...
static void gl_puts(buf) char *buf;
{
  int len;
...
static void gl_error(buf) char *buf;
{
  int len = strlen(buf);
...
char *gl_getline(prompt) char *prompt;
{
  int c, loc, tmp;
...
static void gl_addchar(c) int c;
...
static void gl_del(loc) int loc;
/*
...
static void gl_kill(pos) int pos;
...
static void gl_word(direction) int direction;
...
static void gl_fixup(prompt, change, cursor) char *prompt;
int change, cursor;
/*
 ...
 */
{
  ...
  { /* no scroll, but text changed */
    ...
    new_right =
      (gl_extent && (right > left + gl_extent)) ? left + gl_extent : right;
  }
...
static int gl_tab(buf, offset, loc) char *buf;
int offset;
int *loc;
...
static char *hist_save(p) char *p;
...
static void search_update(c) int c;
{
  if (c == 0)
  {
    search_pos       = 0;
...
static void search_addchar(c) int c;
{
  char *loc;
...
static void search_back(new_search) int new_search;
{
  int found = 0;
  char *p, *loc;
...
static void search_forw(new_search) int new_search;
{
  int found = 0;
  char *p, *loc;
