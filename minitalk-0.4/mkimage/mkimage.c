...
static ObjPtr newCompiledMethod(ObjPtr selector, ObjPtr primitive,
                                ObjPtr numberargs, ObjPtr tempsize,
                                ObjPtr stacksize, ObjPtr bytecodes,
                                ObjPtr literals)
{
...
static ObjPtr newMethodContext(ObjPtr sender, ObjPtr instptr, ObjPtr stackptr,
                               ObjPtr stack, ObjPtr class, ObjPtr method,
...
static void bigBangPart2(void)
{
  ...
  patchClass(machine.false, getPointer(machine.False, VALUE_IN_ASSOCIATION));
  patchClass(machine.true, getPointer(machine.True, VALUE_IN_ASSOCIATION));
  for (i = 0; i < 256; i++)
  {
  ...
  {
    patchClass(symbols, getPointer(machine.LinkedObject, VALUE_IN_ASSOCIATION));
    symbol = getPointer(symbols, OBJECT_IN_LINKEDOBJECT);
    patchClass(symbol, getPointer(machine.Symbol, VALUE_IN_ASSOCIATION));
...
int fillInInstVarNames(ObjPtr class, char *nameString)
{
  char *name;
...
void fillInCharacteristic(ObjPtr class, char *creationKeyword,
                          int totalNumberInstvars)
{
  ...
  setPointer(class, CHARACTERISTIC_IN_CLASS, newSmallInteger(characteristic));
}
...
void fillInClassData(char **tokens)
{
  ...
  patchClass(metaclass, getPointer(machine.Metaclass, VALUE_IN_ASSOCIATION));
  ...
  fillInCharacteristic(class, tokens[1], numberInstvars + numberSuperInstvars);
...
int main(int argc, char *argv[])
{
  ...
  for (i = 1; i < argc; i++)
  {
    if (argv[i][0] == '-')
    {
      ...
      {
        error("unrecognized option '%s'; try '%s --help'", argv[i], argv[0]);
      }
    }
    else
    {
