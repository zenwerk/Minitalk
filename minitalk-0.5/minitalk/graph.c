...
#define RGB2PIXEL(r, g, b) \
  (C2B(r, vga.red) | C2B(g, vga.green) | C2B(b, vga.blue))
...
static void initMonitor(int argc, char *argv[])
{
  ...
  visualInfo = XGetVisualInfo(vga.display, VisualClassMask | VisualScreenMask,
  ...
  vga.image = XCreateImage(vga.display, visual, bestDepth, ZPixmap, 0, NULL,
  ...
  vga.win                 = XCreateWindow(
    vga.display, rootWin, WINDOW_POS_X, WINDOW_POS_Y, WINDOW_SIZE_X,
  ...
  classHints->res_name   = "MiniTalk";
  classHints->res_class  = "MiniTalk";
  if (XStringListToTextProperty(&classHints->res_name, 1, &windowName) == 0)
  {
    error("property allocation failed");
  }
  XSetWMProperties(vga.display, vga.win, &windowName, NULL, argv, argc,
...
static void *server(void *ignore)
{
  ...
  while (run)
  {
    ...
    {
      case Expose:
        XPutImage(vga.display, vga.win, vga.gc, vga.image, event.xexpose.x,
                  event.xexpose.y, event.xexpose.x, event.xexpose.y,
                  event.xexpose.width, event.xexpose.height);
        break;
      case ClientMessage:
        if (event.xclient.message_type == XA_WM_COMMAND &&
            event.xclient.format == 32 && event.xclient.data.l[0] == 0xDEADBEEF)
        {
          run = false;
        }
        break;
...
static int myArgc     = 1;
static char *myArgv[] = {"MiniTalk", NULL};
...
Word graphRead(Word addr)
{
  ...
  if (!installed)
  {
    error("graphics controller not installed");
  }
  if (addr >= WINDOW_SIZE_X * WINDOW_SIZE_Y * 4)
  {
    error("graphics read access out of bounds");
  }
...
void graphWrite(Word addr, Word data)
{
  ...
  if (addr >= WINDOW_SIZE_X * WINDOW_SIZE_Y * 4)
  {
    error("graphics write access out of bounds");
  }
  /* write to frame buffer memory */
  vgaWrite((addr >> 2) % WINDOW_SIZE_X, (addr >> 2) / WINDOW_SIZE_X,
           (data >> 16) & 0xFF, (data >> 8) & 0xFF, (data >> 0) & 0xFF);
}
