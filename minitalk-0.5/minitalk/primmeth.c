...
static FILE *fileArray[MAX_FILES] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL,
...
static Bool prim047(int numArgs)
{
  ...
  class = getClass(getPointer(machine.currentStack, machine.sp - numArgs - 1));
...
static Bool prim248(int numArgs)
{
  /* Graphics >> on */
  graphInit();
  SUCCEED(RCVR);
...
static Bool prim249(int numArgs)
{
  /* Graphics >> off */
  graphExit();
  SUCCEED(RCVR);
...
{
  graphWrite((y * 640 + x) * 4, color);
}

static Bool prim250(int numArgs)
{
  Word color;
  int x, y;

  /* Graphics >> clear: */
  color = smallIntegerValue(ARG(1));
  for (y = 0; y < 480; y++)
  {
    for (x = 0; x < 640; x++)
    {
...
static Bool prim251(int numArgs)
{
  int x0, y0;
  int x1, y1;
  Word color;
  ...
  /* Graphics >> xStart:yStart:xEnd:yEnd:color: */
  x0    = smallIntegerValue(ARG(1));
  y0    = smallIntegerValue(ARG(2));
  x1    = smallIntegerValue(ARG(3));
  y1    = smallIntegerValue(ARG(4));
  color = smallIntegerValue(ARG(5));
  /* Bresenham's famous line drawing algorithm */
  x = x0;
  y = y0;
  setPixel(x, y, color);
  dx = x1 - x0;
...
PrimitiveMethod primitiveMethods[256] = {
  ...
  illPrim, illPrim, illPrim, illPrim, illPrim, illPrim, illPrim, illPrim,
  illPrim, illPrim, illPrim, illPrim, illPrim, illPrim, illPrim, illPrim,
  prim248, prim249, prim250, prim251, illPrim, illPrim, illPrim, illPrim};
