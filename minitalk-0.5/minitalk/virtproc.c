...
void showObject(char *name, ObjPtr object)
{
  ...
  printf("object with pointer 0x%08lX is %s ", object,
         article(getByte(className, 0)));
  printString(className);
  if (getClass(class) == getPointer(machine.Metaclass, VALUE_IN_ASSOCIATION))
  {
    printf(" class");
  }
  ...
  for (i = 0; i < size; i++)
  {
    if (pointers)
    {
      ...
      className = getPointer(class, NAME_IN_CLASS);
      printf("    pointer %u: 0x%08lX (%s ", i, pointer,
             article(getByte(className, 0)));
      printString(className);
      if (getClass(class) ==
...
static Word showInstruction(Word ip, ObjPtr bytecodes, ObjPtr literals)
{
  Byte opcode, value;
  Byte selectorNumber;
  Byte stacksize;
...
static void debug(void)
{
  ...
  while (!back)
  {
    showInstruction(machine.ip, machine.currentBytecodes,
    ...
    do
    {
      ...
      gl_histadd(line);
      c = line[0];
    } while (c != 'i' && c != 'w' && c != 'l' && c != 'n' && c != 'g' &&
             c != 'q');
    switch (c)
    {
      case 'i':
        line = gl_getline(
          "INSPECT: activeCtx, homeCtx, fromCtx, "
        ...
        switch (c)
        {
          case 'a':
            ...
            break;
          case 'h':
            if (machine.currentHomeContext != machine.currentActiveContext)
            {
      ...
      case 'w':
        ...
        break;
      case 'l':
        showWhere(getClass(machine.currentReceiver), machine.currentClass,
                  machine.currentSelector);
        showInstructions(machine.currentBytecodes, machine.currentLiterals);
        break;
      case 'n':
        back = true;
        break;
...
void fetchContextRegisters(void)
{
  ...
  if (contextClass == getPointer(machine.BlockContext, VALUE_IN_ASSOCIATION))
  {
    machine.currentHomeContext =
      getPointer(machine.currentActiveContext, HOME_IN_BLOCKCONTEXT);
  }
  ...
  machine.ip = smallIntegerValue(
    getPointer(machine.currentActiveContext, INSTPTR_IN_CONTEXT));
  ...
  machine.sp = smallIntegerValue(
    getPointer(machine.currentActiveContext, STACKPTR_IN_CONTEXT));
}
...
void executeNewMethod(int numArgs)
{
  ...
  setPointer(machine.newContext, SENDER_IN_METHODCONTEXT,
             machine.currentActiveContext);
  setPointer(machine.newContext, CLASS_IN_METHODCONTEXT, machine.newClass);
  setPointer(machine.newContext, METHOD_IN_METHODCONTEXT, machine.newMethod);
  object = newSmallInteger(0);
  setPointer(machine.newContext, INSTPTR_IN_METHODCONTEXT, object);
  tempsize = smallIntegerValue(
    getPointer(machine.newMethod, TEMPSIZE_IN_COMPILEDMETHOD));
  object = allocateObject(getPointer(machine.Array, VALUE_IN_ASSOCIATION),
                          tempsize, true);
  setPointer(machine.newContext, TEMPORARIES_IN_METHODCONTEXT, object);
  for (i = 1; i <= numArgs; i++)
  {
    setPointer(object, numArgs - i, pop());
  }
  setPointer(machine.newContext, RECEIVER_IN_METHODCONTEXT, pop());
  stacksize = smallIntegerValue(
    getPointer(machine.newMethod, STACKSIZE_IN_COMPILEDMETHOD));
  object = allocateObject(getPointer(machine.Array, VALUE_IN_ASSOCIATION),
                          stacksize, true);
  setPointer(machine.newContext, STACK_IN_METHODCONTEXT, object);
  object = newSmallInteger(0);
  setPointer(machine.newContext, STACKPTR_IN_METHODCONTEXT, object);
    ...
    switch (opcode)
    {
      case HI4(OP_SPECIALS):
        switch (value)
        {
          ...
          case LO4(OP_RET):
            ...
            setPointer(machine.currentHomeContext, SENDER_IN_METHODCONTEXT,
                       machine.nil);
      ...
      case HI4(OP_SENDSUPER):
        ...
        if (opcode != HI4(OP_SENDSUPER))
        {
          ...
          class =
            getClass(getPointer(machine.currentStack, machine.sp - value - 1));
        }
        else
        {
        ...
        if (value != smallIntegerValue(getPointer(
      ...
      case HI4(OP_PUSHBLOCK):
        ...
        machine.newContext =
          allocateObject(getPointer(machine.BlockContext, VALUE_IN_ASSOCIATION),
                         SIZE_OF_BLOCKCONTEXT, true);
        object = allocateObject(getPointer(machine.Array, VALUE_IN_ASSOCIATION),
                                stacksize, true);
        setPointer(machine.newContext, STACK_IN_BLOCKCONTEXT, object);
        object = newSmallInteger(value);
        setPointer(machine.newContext, NUMBERARGS_IN_BLOCKCONTEXT, object);
        ...
        setPointer(machine.newContext, INITIALIP_IN_BLOCKCONTEXT, object);
        setPointer(machine.newContext, HOME_IN_BLOCKCONTEXT,
                   machine.currentHomeContext);
