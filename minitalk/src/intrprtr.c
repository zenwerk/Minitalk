...
void showObject(char *name, ObjPtr object)
{
  ...
  if (getClass(clazz) == getPointer(machine.Metaclass, VALUE_IN_ASSOCIATION))
  {
    printf(" class");
  }
  ...
  for (i = 0; i < size; i++)
  {
    if (pointers)
    {
      ...
      className = getPointer(clazz, NAME_IN_CLASS);
      printf("    pointer %lu: 0x%08lX (%s ", i, pointer,
             article(getByte(className, 0)));
      printString(className);
      if (getClass(clazz) ==
...
Ushort showInstruction(Ushort ip, ObjPtr bytecodes, ObjPtr literals)
{
  Uchar opcode, value;
