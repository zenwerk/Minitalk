...
ObjPtr allocateObject(ObjPtr clazz, Ulong size, Boolean hasPointers)
{
  ...
  if (hasPointers)
  {
    ...
    for (i = 0; i < size; i++)
    {
      ...
      writeObjPtr(object + sizeof(ObjPtr) + sizeof(Ulong) + i * sizeof(ObjPtr),
                  machine.nil);
    }
  }
  else
  {
    ...
    for (i = 0; i < size; i++)
    {
      ...
      writeUchar(object + sizeof(ObjPtr) + sizeof(Ulong) + i * sizeof(Uchar),
                 0);
    }
  }
...
void setByte(ObjPtr object, Ulong index, Uchar value)
{
  ...
  writeUchar(object + sizeof(ObjPtr) + sizeof(Ulong) + index * sizeof(Uchar),
             value);
}
...
void setPointer(ObjPtr object, Ulong index, ObjPtr value)
{
  ...
  writeObjPtr(object + sizeof(ObjPtr) + sizeof(Ulong) + index * sizeof(ObjPtr),
              value);
}
...
void initMemory(char *fileName)
{
  ...
  if (machine.majorVersion != MAJOR_VNUM || machine.minorVersion != MINOR_VNUM)
  {
